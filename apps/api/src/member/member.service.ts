import { HttpException, HttpStatus, Inject, Injectable, NotFoundException, ServiceUnavailableException } from "@nestjs/common";
...
export class MemberService {
  private readonly lookupAttempts = new Map<string, LookupAttempt>();

  constructor(@Inject(SUPABASE_ADMIN) private readonly supabase: SupabaseClient) {}

  async lookup(phone: string, clientAddress: string) {
    const normalizedPhone = phone.replace(/\s/g, "");
    this.enforceLookupLimit(`ip|${clientAddress}`, 30);
    this.enforceLookupLimit(`phone|${clientAddress}|${normalizedPhone}`, LOOKUP_LIMIT);
    ...
    const customerResult = await this.supabase
      ...
      .maybeSingle();
    if (customerResult.error) throw new ServiceUnavailableException("Không thể tải Thẻ MING lúc này");
    if (!customerResult.data) throw new NotFoundException("Không tìm thấy Thẻ MING cho số điện thoại này");
    ...
    const [appointmentsResult, referralsResult] = await Promise.all([
      this.supabase
        .from("appointments")
        .select("booking_code,starts_at,ends_at,status,total_amount,barbers(name),appointment_services(service_name,duration_minutes,unit_price,quantity,sort_order)")
    ...
    if (appointmentsResult.error || referralsResult.error) {
      throw new ServiceUnavailableException("Không thể tải thông tin Thẻ MING lúc này");
    }

    const appointments = ((appointmentsResult.data ?? []) as DatabaseRow[]).map(toMemberAppointment);
    const now = Date.now();
    const upcomingAppointments = appointments
      .filter((appointment) => appointment.startsAt > now && !["completed", "cancelled", "no_show"].includes(appointment.status))
      .sort((a, b) => a.startsAt - b.startsAt);
    const appointmentHistory = appointments
      .filter((appointment) => !upcomingAppointments.some((upcoming) => upcoming.bookingCode === appointment.bookingCode))
  ...
  private enforceLookupLimit(key: string, limit: number) {
    const now = Date.now();
    const current = this.lookupAttempts.get(key);
    if (!current || current.resetsAt <= now) {
      this.lookupAttempts.set(key, { count: 1, resetsAt: now + LOOKUP_WINDOW_MS });
      this.removeExpiredAttempts(now);
      return;
    }
    if (current.count >= limit) {
      throw new HttpException("Bạn đã tra cứu quá nhiều lần. Vui lòng thử lại sau 15 phút.", HttpStatus.TOO_MANY_REQUESTS);
    }
    current.count += 1;
  }

  private removeExpiredAttempts(now: number) {
    if (this.lookupAttempts.size < 1000) return;
    this.lookupAttempts.forEach((attempt, key) => {
      if (attempt.resetsAt <= now) this.lookupAttempts.delete(key);
    });
  }
}
