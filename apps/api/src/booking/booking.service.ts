...
export class BookingService {
  ...
  async catalog() {
    const [locationResult, settingsResult, servicesResult, serviceLocationsResult] = await Promise.all([
      this.supabase.from("locations").select("id,name,slug,timezone,phone,address").eq("id", LOCATION_ID).eq("active", true).single(),
      this.supabase
        .from("location_settings")
        .select("booking_window_days,booking_slot_minutes,capacity_per_barber,allow_any_barber")
        .eq("location_id", LOCATION_ID)
        .single(),
      this.supabase
        .from("services")
        .select("id,name,description,duration_minutes,price_amount,sort_order")
        .eq("active", true)
        .is("archived_at", null)
        .order("sort_order"),
      this.supabase
    ...
    throwOnErrors(locationResult, settingsResult, servicesResult, serviceLocationsResult);
    if (!locationResult.data || !settingsResult.data) throw new BadRequestException("Chi nhánh chưa được cấu hình");
    const overrides = new Map(
      ((serviceLocationsResult.data ?? []) as DatabaseRow[]).map((row) => [String(row.service_id), row])
    );
  ...
  async availability(date: string) {
    ...
    const [barbersResult, locationsResult, shiftsResult, appointmentsResult] = await Promise.all([
      this.supabase.from("barbers").select("id").eq("active", true).is("archived_at", null),
      this.supabase.from("barber_locations").select("barber_id").eq("location_id", LOCATION_ID).eq("active", true),
      this.supabase
        .from("work_shifts")
    ...
    throwOnErrors(barbersResult, locationsResult, shiftsResult, appointmentsResult);
    const activeBarbers = new Set(((barbersResult.data ?? []) as DatabaseRow[]).map((row) => String(row.id)));
    const locationBarbers = new Set(((locationsResult.data ?? []) as DatabaseRow[]).map((row) => String(row.barber_id)));
    const shifts = ((shiftsResult.data ?? []) as DatabaseRow[]).filter(
      (row) => activeBarbers.has(String(row.barber_id)) && locationBarbers.has(String(row.barber_id))
    );
    const appointments = (appointmentsResult.data ?? []) as DatabaseRow[];
    const slots = new Map<string, { from: string; to: string; barberIds: Set<string> }>();
    ...
    shifts.forEach((shift) => {
      ...
      while (cursor + settings.slotMinutes <= end) {
        ...
        const slot = slots.get(key) ?? { from: slotFrom, to: slotTo, barberIds: new Set<string>() };
      ...
      slots: Array.from(slots.values())
        .map((slot) => {
          ...
          return {
            time: slot.from,
            endsAt: slot.to,
            remainingCapacity: Math.max(slot.barberIds.size * settings.capacityPerBarber - booked, 0)
          };
        })
        .filter((slot) => slot.remainingCapacity > 0 && new Date(`${date}T${slot.time}:00${TIMEZONE_OFFSET}`).getTime() > now)
  ...
  async availableBarbers(date: string, time: string, serviceIds: string[]) {
    ...
    if (new Date(startsAt).getTime() <= Date.now()) throw new BadRequestException("Khung giờ này đã qua");
    const endsAt = addMinutesIso(startsAt, settings.slotMinutes);

    const [servicesResult, serviceLocationsResult, barbersResult, barberLocationsResult, skillsResult, shiftsResult, appointmentsResult] = await Promise.all([
      this.supabase.from("services").select("id").in("id", serviceIds).eq("active", true).is("archived_at", null),
      this.supabase.from("service_locations").select("service_id").eq("location_id", LOCATION_ID).eq("active", true).in("service_id", serviceIds),
      this.supabase.from("barbers").select("id,name,bio,metadata,sort_order").eq("active", true).is("archived_at", null).order("sort_order"),
      this.supabase.from("barber_locations").select("barber_id").eq("location_id", LOCATION_ID).eq("active", true),
      this.supabase.from("barber_services").select("barber_id,service_id").eq("active", true).in("service_id", serviceIds),
      this.supabase
        .from("work_shifts")
        .select("barber_id")
        .eq("location_id", LOCATION_ID)
    ...
    throwOnErrors(servicesResult, serviceLocationsResult, barbersResult, barberLocationsResult, skillsResult, shiftsResult, appointmentsResult);
    if ((servicesResult.data ?? []).length !== serviceIds.length || (serviceLocationsResult.data ?? []).length !== serviceIds.length) {
      throw new BadRequestException("Có dịch vụ không còn hoạt động");
    }

    const locationIds = new Set(((barberLocationsResult.data ?? []) as DatabaseRow[]).map((row) => String(row.barber_id)));
    const shiftIds = new Set(((shiftsResult.data ?? []) as DatabaseRow[]).map((row) => String(row.barber_id)));
    const skills = (skillsResult.data ?? []) as DatabaseRow[];
    const appointments = (appointmentsResult.data ?? []) as DatabaseRow[];
    const eligible = ((barbersResult.data ?? []) as DatabaseRow[]).filter((barber) => {
      const id = String(barber.id);
      return locationIds.has(id) && shiftIds.has(id) && serviceIds.every((serviceId) =>
        skills.some((skill) => String(skill.barber_id) === id && String(skill.service_id) === serviceId)
      );
    });
    const totalRemaining = Math.max(eligible.length * settings.capacityPerBarber - appointments.length, 0);

    return {
      allowAnyBarber: settings.allowAnyBarber && totalRemaining > 0,
      barbers: eligible
        .map((barber) => {
          const assigned = appointments.filter((appointment) => String(appointment.barber_id) === String(barber.id)).length;
          return {
            id: barber.id,
            name: barber.name,
            detail: barber.bio ?? "",
            avatarUrl: metadataAvatar(barber.metadata),
            remainingCapacity: Math.max(Math.min(settings.capacityPerBarber - assigned, totalRemaining), 0)
          };
        })
  ...
  async createAppointment(input: CreateWebsiteBookingDto) {
    ...
    if (error) throw new BadRequestException(humanizeBookingError(error.message));
    return data;
  }
  ...
  private async getSettings(): Promise<LocationSettings> {
    const { data, error } = await this.supabase
      ...
      .eq("location_id", LOCATION_ID)
      .single();
    if (error || !data) throw new BadRequestException(error?.message ?? "Chi nhánh chưa được cấu hình");
    return {
...
function throwOnErrors(...results: Array<{ error: { message: string } | null }>) {
...
function humanizeBookingError(message: string) {
  if (message.includes("Khung giờ này đã đủ") || message.includes("Barber đã đủ")) return "Khung giờ vừa hết chỗ. Vui lòng chọn giờ khác.";
  return message;
}
